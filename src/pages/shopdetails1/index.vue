<template>
  ...
  <div class='top'>
    ...
    <p class='price'>
      <span style='font-size:20px;color:red'>￥{{sale_price}}</span><span><van-icon style='margin-top:2px;float:left' name="orders-o" />库存{{shop.stocknum}}</span>
    </p>
  ...
  <div class='footer'></div>
    <van-goods-action>
     ...
     <!-- <van-goods-action-button text="加入购物车" @click="addShopCart()"  type="warning" />-->
      <van-goods-action-button style='background:#f1f1f1;color:#333'  v-if='shop.stocknum==0' text="库存不足"/>
      <van-goods-action-button v-else text="立即购买" @click="allligm()"/>
    ...
    <van-popup :show="show" @close="onClose" position='bottom'>
      <div class='sku_shop'>
        ...
        <div>
          ...
          <p style='color:#aeaeae'><span>库存<van-icon style='margin:2px 5px 0px;float:left' name="orders-o" />{{shop.stocknum}}</span></p>
        </div>
      ...
      <div class='sku_category'>
        ...
        <div class='sku_foot'>
          ...
          <p v-if='shop.stocknum==0' class='ljgm'>库存不足</p>
          <p v-else class='ljgm' @click='allligm'>立即购买</p>
        </div>
      </div>
    </van-popup>
  </div>
...
export default {
  ...
  onLoad() {
      this.sale_price = this.$mp.query.price
      this.fenleiid='';
  ...
  methods: {
    ...
    allligm(){
      this.show = false
        ...
        this.$go('placeorder',{type:1,id:this.shop.id,sku:this.fenleiid,num:1,shopType:2,title:this.shop.goods_name,img:this.shop.image_uri,price:this.sale_price,msid:this.$mp.query.msid})
    },
     // 商品详情
    shaopInfo(){
      this.$wxAjax('spikeDetail', {
        userToken: this.$getStorage('token'),
        goods_id:this.$mp.query.id,
        id:this.$mp.query.msid
        // goods_id:5
      }).then((data) => {
        if (data.status == 0) {
          this.shop = data.data;
